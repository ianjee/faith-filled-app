  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        const avgPain = painRows.length
          ? Math.round(
              painRows.reduce(
                (sum, r) =>
                  sum +
                  ((r.pain_before - r.pain_after) / r.pain_before) * 100,
                0
              ) / painRows.length
            )
          : null;
        ...
        const recPct = rows.length
          ? Math.round((recCount / rows.length) * 100)
          : null;

        const listenedCount = rows.filter(
          (r) => r.felt_listened_to === true
        ).length;
        const listenedPct = rows.length
          ? Math.round((listenedCount / rows.length) * 100)
          : null;
  ...
  return (
    <Screen>
      <ScrollView
        ...
        contentContainerStyle={{ paddingTop: 30, paddingBottom: 34 }}
      >
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 9, marginBottom: 14 }}>
          ...
          <View style={{ width: "48%" }}>
            <StatCard
              ...
              value={
                metrics.avgPainImprovementPct !== null
                  ? `${metrics.avgPainImprovementPct}%`
                  : "—"
              }
            />
          </View>
          ...
          <View style={{ width: "48%" }}>
            <StatCard
              ...
              value={
                metrics.wouldRecommendPct !== null
          ...
          <View style={{ width: "48%" }}>
            <StatCard
              ...
              value={
                metrics.feltListenedToPct !== null
                  ? `${metrics.feltListenedToPct}%`
                  : "—"
              }
            />
          </View>
        ...
        <Card>
          <Text style={{ color: COLORS.inkMid, lineHeight: 19 }}>
            These figures come from client outcome surveys — pain before/after,
            and likelihood to recommend — rather than a single star rating,
            matching the outcome-based approach in the initial plan.
          </Text>
        </Card>
